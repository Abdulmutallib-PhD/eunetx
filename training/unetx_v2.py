...
def visualize_outputs(outputs: List[torch.Tensor], input_tensor: torch.Tensor):
    """
    Visualizes a central slice from each deep supervision output and the original input.
    """
    input_np = input_tensor.detach().cpu().squeeze().numpy()
    center_slice_idx = input_np.shape[0] // 2
    input_slice = input_np[center_slice_idx, :, :]
    ...
    for i, output in enumerate(outputs):
        ...
        center_slice_idx = output_np.shape[0] // 2
        output_slice = output_np[center_slice_idx, :, :]

        plt.subplot(1, len(outputs) + 1, i + 2)
        plt.imshow(output_slice, cmap='hot')
...
if __name__ == '__main__':
    ...
    visualize_outputs(predictions, dummy_input)
